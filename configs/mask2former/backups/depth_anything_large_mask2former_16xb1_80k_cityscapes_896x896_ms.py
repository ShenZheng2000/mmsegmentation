# # NOTE: this code has shape errors, skip for now
# _base_ = [
#     '../_base_/default_runtime.py', '../_base_/datasets/cityscapes.py'
# ]

# # NOTE: add data_root here
...
# train_dataloader = dict(batch_size=1, dataset=dict(pipeline=train_pipeline, data_root=data_root)) # NOTE: mention data_root here
# val_dataloader = dict(dataset=dict(pipeline=test_pipeline, data_root=data_root)) # NOTE: mention data_root here
...
# NOTE: I try this simpler version, does not work. Debug later. 
# _base_ = [
#     'depth_anything_large_mask2former_16xb1_80k_cityscapes_896x896.py'  # Update with the actual path to the source file
# ]

# # Additional or modified configurations for multi-scale testing

# data_preprocessor = dict(
#     test_cfg=dict(size_divisor=14)  # Adding this specific configuration
# )

# # Update or add the test time augmentation (TTA) pipeline
# img_ratios = [0.5, 0.75, 1.0, 1.25, 1.5, 1.75, 2.0]
# tta_pipeline = [
#     dict(type='LoadImageFromFile', backend_args=None),
#     dict(
...
# # Specify only the unique part of default_hooks
# default_hooks = dict(
#     logger=dict(type='LoggerHook', interval=1, log_metric_by_epoch=False)  # More frequent logging
# )
